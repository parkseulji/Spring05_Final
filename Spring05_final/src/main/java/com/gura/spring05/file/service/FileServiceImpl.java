...
public class FileServiceImpl implements FileService{
	...
	public void list(HttpServletRequest request) {	
		 ...
		 * - 전달 안 되는 경우:navbar에서 파일 목록보기를 누른 경우
		 * - 전달 되는 경우:하단에 검색어를 입력하고 검색 버튼을 누른 경우
		 * - 전달 되는 경우2:이미 검색을 한 상태에서 하단 페이지 번호를 누른 경우
		 */
		//검색과 관련 된 파라미터를 읽어 와 본다.
		String keyword=request.getParameter("keyword");
		String condition=request.getParameter("condition");
		
		//검색 키워드가 존재 한다면 키워드를 담을 FileDto 객체 생성
	...
	public void removeFile(HttpServletRequest request) {
		...
		FileDto dto=dao.getData(num);
		//파일 작성자와 로그인 된 아이디가 다르면 예외를 발생시킨다.(이 밑으로는 수행하지 않음.)
		String id=(String)request.getSession().getAttribute("id");
		if(!id.equals(dto.getWriter())) { //java에서는 equals 메소드를 이용하여 비교
